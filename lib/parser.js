...
module.exports = {
  ...
  parseRequest: function parseRequest(request, options) {
        ...
        case State.Params:
          switch (Number(substate)) {
            ...
            case ParamsState.Number:
              if (c === ',') {
                parsed.params[tmpName] = parseInt(tmpValue);
                tmpName = '';
    ...
    if (parsed.params.created) {
      var skew = parsed.params.created  - Math.floor(Date.now() / 1000);
    ...
    if (parsed.params.expires) {
      ...
      if (expiredSince > options.clockSkew) {
        throw new ExpiredRequestError('Request expired with skew ' + expiredSince +
          's greater than allowed ' + options.clockSkew + 's');
      }
    }
