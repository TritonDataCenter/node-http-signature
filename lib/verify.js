...
module.exports = {
   ...
   */
  verifySignature: function(parsedSignature, key) {
    if (!parsedSignature || typeof(parsedSignature) !== 'object')
      throw new TypeError('parsedSignature is required');

    var alg = parsedSignature.algorithm.match(/(HMAC|RSA)-(\w+)/);
    if (!alg || alg.length !== 3)
